/**************************************************************************/
/*  This file is part of POPCON.                                          */
/*                                                                        */
/*  Copyright (C) 2025                                                    */
/*    CEA (Commissariat à l'énergie atomique et aux énergies              */
/*         alternatives)                                                  */
/*                                                                        */
/*  you can redistribute it and/or modify it under the terms of the GNU   */
/*  Lesser General Public License as published by the Free Software       */
/*  Foundation, version 2.1.                                              */
/*                                                                        */
/*  It is distributed in the hope that it will be useful,                 */
/*  but WITHOUT ANY WARRANTY; without even the implied warranty of        */
/*  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the         */
/*  GNU Lesser General Public License for more details.                   */
/*                                                                        */
/*  See the GNU Lesser General Public License version 2.1                 */
/*  for more details (enclosed in the file licenses/LGPLv2.1).            */
/*                                                                        */
/**************************************************************************/

//! Solve popcon on CNF with `dc-ssat`

use crate::cnf::{CNFPopularityContest, ModelCount, PopConBounds};
use crate::model::Model;
use crate::utils::LastLines;
use crate::utils::{Input, MaybePersistentTempFile};
use anyhow::Context;
use fixedbitset::FixedBitSet;
use itertools::Itertools;
use nom::Parser;
use num_bigint::BigInt;
use num_rational::BigRational;
use num_traits::One;
use std::io::{BufRead, BufWriter};
use std::io::{Read, Write};
use std::os::unix::prelude::{CommandExt, ExitStatusExt};

/// Runs `dcssat ./formula`
pub fn run_dcssat(
    cnf_formula: impl Input,
    controlled_variables: &FixedBitSet,
) -> anyhow::Result<(Model, PopConBounds)> {
    let mut input = std::io::BufReader::new(cnf_formula.to_read()?);
    let mut header = String::new();
    input
        .read_line(&mut header)
        .context("reading formula header")?;
    let mut formula =
        MaybePersistentTempFile::new(".sdimacs").context("creating temp file for dcssat input")?;
    let mut fmt = BufWriter::new(&mut formula);
    writeln!(
        &mut fmt,
        "{}e {} 0",
        header,
        controlled_variables.ones().map(|x| x + 1).format(" ")
    )
    .context("writing dcssat input")?;
    let mut complement = controlled_variables.clone();
    complement.toggle_range(..);
    writeln!(
        &mut fmt,
        "r 0.5 {} 0",
        complement.ones().map(|x| x + 1).format(" ")
    )
    .context("writing dcssat input")?;
    let mut buf = [0; 1024];
    loop {
        let n = input
            .read(&mut buf)
            .with_context(|| format!("reading {}", cnf_formula.display()))?;
        if n == 0 {
            break;
        }
        fmt.write_all(&buf[..n]).context("writing to temp file")?;
    }
    fmt.flush().context("flushing temp file")?;
    drop(fmt);
    let mut cmd = std::process::Command::new("dcssat");
    cmd.arg(formula.as_ref())
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped());
    unsafe {
        cmd.pre_exec(|| prctl::set_death_signal(9).map_err(std::io::Error::from_raw_os_error))
    };
    let process_span = tracing::trace_span!("running dcssat", timing = true).entered();
    tracing::trace!("dcssat command: {:?}", &cmd);
    let mut process = cmd.spawn().context("running dcssat")?;
    let mut reader = crate::ddnnf::parser::FileSource::new(process.stdout.take().unwrap());
    let stderr = LastLines::new(process.stderr.take().unwrap());
    let bits = controlled_variables.len();
    let uncontrolled_bits = bits - controlled_variables.count_ones(..);
    let offset = BigInt::one() << uncontrolled_bits;
    let offset = BigRational::from_integer(offset);
    const EXACT: &'static [u8] = b"Pr[SAT]";
    let model_count = loop {
        /// Like nom::bytes::streaming::tag but does not borrow the parsed string.
        fn static_tag<'a>(
            tag: &'static [u8],
        ) -> impl FnMut(&'a [u8]) -> nom::IResult<&'a [u8], &'static [u8]> {
            nom::combinator::value(tag, nom::bytes::streaming::tag(tag))
        }
        let typ = reader
            .feeder
            .parse_and_advance(|s: &[u8]| static_tag(EXACT).parse(s));
        match typ {
            Ok(EXACT) => {
                reader.whitespace().context("Pr[SAT]...")?;
                reader
                    .feeder
                    .parse_and_advance(|s| static_tag(b"=")(s))
                    .context("=")?;
                reader.whitespace().context("Pr[SAT] =...")?;
                let float = reader
                    .feeder
                    .parse_and_advance(|s| nom::number::streaming::double(s))
                    .context("double parsing")?;
                let mut value = BigRational::from_float(float).context("parsed nan")?;
                value *= offset;
                let value = value
                    .round()
                    .to_integer()
                    .to_biguint()
                    .context("negative model count")?;
                break value;
            }
            Ok(_) => unreachable!(),
            Err(_) => {}
        };
        reader
            .feeder
            .parse_and_advance(|s| {
                nom::combinator::map(nom::bytes::streaming::take_until(b"\n" as &[u8]), |_| ())(s)
            })
            .context("consuming rest of line")?;
        reader.whitespace().context("next line")?;
    };
    let status = process.wait().context("waiting for dcssat")?;
    drop(process_span);
    let (err, _) = stderr.get();
    anyhow::ensure!(
        status.success(),
        "dcssat failed with code {:?} signal {:?}: {}",
        status.code(),
        status.signal(),
        String::from_utf8_lossy(&err)
    );
    let bounds = PopConBounds::exactly(ModelCount {
        model_count,
        bits: uncontrolled_bits,
    });
    let model = Model::empty(bits);
    Ok((model, bounds))
}

/// Use dcssat to solve popcon approximately (output is a float).
///
/// Model is not parsed and always 0.
pub struct DCSSAT;

#[test]
fn simple() {
    let mut what = FixedBitSet::with_capacity(3);
    what.insert(0);
    // 3 & (1|2)
    let (_, bounds) = DCSSAT
        .popularity_contest("assets/simple.cnf".as_ref() as &std::path::Path, &what, &FixedBitSet::with_capacity(FixedBitSet::len(&what)))
        .context("counting models")
        .unwrap();
    assert_eq!(
        bounds,
        PopConBounds::exactly(ModelCount {
            model_count: 2u8.into(),
            bits: 2
        })
    );
}

#[test]
fn unsat2() {
    let mut what = FixedBitSet::with_capacity(2);
    what.insert(0);
    let (_, bounds) = DCSSAT
        .popularity_contest(
            "p cnf 2 3
1 0
-1 0
2 0
"
            .as_bytes(),
            &what,
            &FixedBitSet::with_capacity(FixedBitSet::len(&what))
        )
        .context("running ssat")
        .unwrap();
    assert_eq!(
        bounds,
        PopConBounds::exactly(ModelCount {
            model_count: 0u8.into(),
            bits: 1
        })
    );
}

#[test]
fn robust() {
    let mut what = FixedBitSet::with_capacity(2);
    what.insert(0);
    let (_, bounds) = DCSSAT
        .popularity_contest(
            "p cnf 2 1
1 2 0
"
            .as_bytes(),
            &what,
            &FixedBitSet::with_capacity(FixedBitSet::len(&what))
        )
        .context("running ssat")
        .unwrap();
    assert_eq!(
        bounds,
        PopConBounds::exactly(ModelCount {
            model_count: 2u8.into(),
            bits: 1
        })
    );
}

impl CNFPopularityContest for DCSSAT {
    fn popularity_contest(
        &self,
        input: impl Input,
        controlled_variables: &FixedBitSet,
        _: &FixedBitSet
    ) -> anyhow::Result<(Model, PopConBounds)> {
        let (model, count) = run_dcssat(input, controlled_variables)
            .with_context(|| format!("running dcssat on {}", input.display()))?;
        Ok((model, count))
    }
}
